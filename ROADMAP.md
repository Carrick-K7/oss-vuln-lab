...
`oss-vuln-lab` is evolving toward a local-first OSS vulnerability research and verification platform. This file records the long-term direction only. Near-term execution lives in the current working context and git history, and high-impact technical choices live in `docs/decisions/`.
